...
public class MediaController {
    ...
    public ResponseEntity<?> confirmUpload(@RequestBody MediaRequest mediaRequest) {
        try {
            Media media = mediaService.saveMetadata(mediaRequest);
            return ResponseEntity.ok(media);
        } catch (Exception e) {
            logger.error("Failed to confirm upload: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("error", "Failed to confirm upload", "details", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getPresignedUrl(
        ...
        try {
            ...
            String url = mediaService.generatePreSignedUploadUrl(filename, folder, contentType).toString();
