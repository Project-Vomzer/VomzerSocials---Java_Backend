...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    public Mono<RegisterUserResponse> registerWithStandardLogin(StandardRegisterRequest request) {
        ...
        // Generate a synthetic userId for zkLogin compatibility
        String userId = UUID.randomUUID().toString();
        log.debug("Generated userId for standard login: {}", userId);
...
//                    Map<String, String> requestBody = Map.of("username", request.getUserName(), "jwt", request.getJwt());
//                    HttpEntity<Map<String, String>> mapHttpEntity = new HttpEntity<>(requestBody, headers);
//
...
//                                Map<String, Object> walletDetails = response.getBody();
//                                log.info("Node.js service response - Status Code: {}, Body: {}", statusCode, walletDetails);
//
...
//                                    ));
//                                } else {
//                                    return Mono.error(new IllegalArgumentException("Failed to communicate with Node.js service, status code: " + statusCode));
//                                }
//                            });
//                })
//                .onErrorMap(e -> new IllegalArgumentException("zkRegistration failed: " + e.getMessage(), e));
//    }
    ...
    public Mono<RegisterUserResponse> registerWithZkLogin(ZkRegisterRequest request) {
        ...
        return findExistingUserById(userId)
                .flatMap(existingUser -> {
                    ...
                    // Use WebClient for non-blocking HTTP call
...
//                    Check this well\
                    String salt = request.getPassword();
                    return walletApiClient.generateSuiAddress()
                            .flatMap(address -> createUser(request.getUserName(), request.getPassword(), address, null, salt, Role.ZKSocials)
                                    .map(user -> registerNewUserResponse(request.getUserName(), user)));
                })
    ...
    public Mono<LoginResponse> loginWithZkLogin(ZkLoginRequest request) {
        ...
        // Extract user identifier from JWT (e.g., 'sub' claim)
        String userId;
        try {
            userId = jwtUtil.extractUsername(request.getJwt()); // Assumes JwtUtil extracts 'sub' or similar
            log.debug("Extracted userId from JWT: {}", userId);
        } catch (Exception e) {
            log.error("Failed to parse JWT: {}", e.getMessage(), e);
            return Mono.error(new IllegalArgumentException("Invalid JWT format"));
        ...
        // Find user by userId (modify findExistingUser to use userId)
        return findExistingUserById(userId)
                .flatMap(existingUser -> {
                    ...
                    // Call Node.js service with WebClient
    ...
    private Mono<User> createUser(String username, String password, String suiAddress, String publicKey, String salt, Role role) {
        return Mono.fromCallable(() -> {
            User user = new User();
            ...
            user.setSalt(salt);
            user.setIsLoggedIn(false);
            log.info("Saving user: username={}, suiAddress={}, publicKey={}, salt={}",
                    username, suiAddress, publicKey, salt);
            try {
